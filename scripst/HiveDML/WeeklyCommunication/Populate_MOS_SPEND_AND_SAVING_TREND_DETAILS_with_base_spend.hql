...
INSERT OVERWRITE TABLE mosdev.mos_spend_and_saving_trend_details_with_base_spend PARTITION (baao_fiscal_period,week_num)
select
...
AND (date_add(pw2y.fiscal_week_start_date,${week_num})>=TO_DATE(hist_snap.VALID_FROM_DATE) AND date_add(pw2y.fiscal_week_start_date,${week_num})<= TO_DATE(hist_snap.VALID_TO_DATE))
